...
// Та же механика захвата лида, что на /proekty/ust-luga-izhs
// (InvestorForm) — обобщена под произвольный ЖК по slug, без хардкода.
export function ZhkPresentationForm({ slug, title }: { slug: string; title: string }) {
  ...
  const downloadFilename = `${slug}-presentation.pdf`;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/api/zhk-presentation/${slug}`, {
        method: "POST",
      ...
      if (contentType.startsWith("application/pdf")) {
        ...
        // Сразу стартуем скачивание — кнопка остаётся на странице как
        // запасной вариант, если браузер заблокировал автоскачивание.
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2 text-[12px] leading-snug text-ink-secondary">
        <input
          type="checkbox"
          id={`zhk-presentation-consent-${slug}`}
          checked={consent}
          onChange={(e) => setConsent(e.target.checked)}
          className="mt-0.5 accent-accent"
        />
        <label htmlFor={`zhk-presentation-consent-${slug}`}>
      ...
      >
        {status === "submitting" ? "Отправляю…" : `Получить презентацию «${title}» (PDF)`}
      </button>
