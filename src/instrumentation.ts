export async function register() {
  // Временный диагностический лог: проверяем сам факт вызова register() в
  // проде на Timeweb — до какой-либо другой логики и до runtime-проверки
  // ниже, чтобы никакое условие не могло его отфильтровать. Если эта
  // строка не появится в "Логи приложения" после честного передеплоя —
  // значит либо register() там не вызывается вообще (сборка/раннер
  // Timeweb не запускает наш instrumentation.ts), либо "Логи приложения"
  // не показывают stdout процесса, который реально обслуживает трафик.
  // Убрать после того, как источник проблемы найден.
