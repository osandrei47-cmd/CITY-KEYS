...
async function main() {
  ...
  for (const [index, lot] of LUGA_PARK_LOTS.entries()) {
    ...
    if (existing.docs.length) {
      console.log(
        `• «${lot.title}» уже есть (id=${existing.docs[0].id}) — пропускаю. ` +
          "Если описание нужно обновить (напр. правка формулировок) — отредактируйте в админке или удалите лот и запустите скрипт снова.",
      );
      continue;
    }
    ...
    const description = [
      ...
      "",
      "Электричество — ЛЭП по границе участка, подключение оформляется отдельно. Внутренние проезды укатаны щебнем, круглогодичный проезд. Река Луга — вблизи посёлка.",
      "",
